  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[100dvh] p-4 relative z-10 w-full overflow-hidden">
      <div className="w-full max-w-md glass-card rounded-[2rem] p-10 flex flex-col relative overflow-hidden bg-white/50 dark:bg-black/20">
        ...
        <h1 className="text-4xl font-heading font-black text-center mb-10 text-zinc-900 dark:text-zinc-50 tracking-tight dark:text-glow">Create Account</h1>
        ...
        <form onSubmit={handleSignup} className="flex flex-col gap-6">
          <div className="flex flex-col gap-2">
            <label className="text-xs font-bold uppercase tracking-[0.2em] text-zinc-500 dark:text-zinc-400">Your Name</label>
            <input 
              type="text" 
              required
        ...
        <p className="mt-8 text-center text-sm font-medium text-zinc-500 dark:text-zinc-400">
          Already have an account? <Link href="/login" className="text-cyan-600 dark:text-cyan-400 hover:text-cyan-500 dark:hover:text-cyan-300 dark:hover:text-glow transition-all ml-2">Log in</Link>
        </p>
      </div>
    </div>
  );
